...
def _edge_list_1(kg, n_entity, hop):
    ...
    return [(h, t, relation_idx[r]) for h, t, r in edge_list if relation_cnt[r] > 1000], relation_cnt
...
#     one_hops_neighbors = [t[1] for t in kg[entity_id] if t[1] != entity_id and relation_counts[int(t[0])] > 1000]
    ...
    two_hops_neighbors = [t[1] for n_id in one_hops_neighbors for t in kg[n_id]  if t[1] != entity_id and relation_counts[t[0]] > 1000]
...
class dataset(object):
    ...
    def data_process(self,is_finetune=False):
        ...
        for line in self.data:
            ...
            data_set.append([np.array(context),c_lengths,np.array(response),r_length,np.array(mask_response),mask_r_length,final_entity,
